...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        setName: [
          { required: true, message: "数据集合名称必填", trigger: "blur" }
        ],
        setCode: [
          { required: true, message: "数据集合编码必填", trigger: "blur" },
      ...
      setType: "", //数据集合类型，主要用于区分http   addSql  addHttp  edit
      httpForm: {
        //http数据源相关数据
        apiUrl: "",
        method: "GET",
      ...
      rules: {
        sourceType: [
          { required: true, message: "数据集合名称必选", trigger: "change" }
        ],
        sourceCode: [
          { required: true, message: "数据集合编码必填", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    async addOrEditDataSet(row, type) {
      ...
      if (row.id) {
        ...
        this.dialogFormVisibleTitle = "编辑数据集合";
        ...
        this.dialogFormVisibleTitle = "新增数据集合";
    ...
    async handleClickTabs(tab, event) {
      if (this.setType == "http") {
        //针对http数据源
        console.log("http数据集合" + this.httpForm);
